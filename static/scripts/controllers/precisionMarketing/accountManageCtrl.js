...
function (exportService) {
    ...
    function ($scope, httpService, Notification, $confirm, $uibModal) {
        ...
        $scope.searchShow = function () {
            if ($scope.userAccountName == "" || $scope.userAccountName == null) {
        ...
        var regTel = /^((0\d{2,3}-\d{7,8})|(1\d{10})|(0\d{2,3}-\d{7,8}-\d{1,5}))$/;
        ...
        function ValidInput() {  
            ...
            if (!(regTel.test($scope.Contant_Infor1))) {
                Notification.error({ message: "联系方式1格式不正确，请输入正确的联系方式", delay: 1000 });
                return false;
            }
