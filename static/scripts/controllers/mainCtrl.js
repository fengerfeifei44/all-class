define(['scripts/services/httpService',
    ...
    'scripts/requireHelper/requireConfirm'], function () {
        return ['$scope', '$timeout', '$location', 'httpService', '$confirm', 'Notification', '$window', '$state', function ($scope, $timeout, $location, httpService, $confirm, Notification, $window, $state) {
            ...
            httpService.post('DAAS', 'MenuService', 'GetCurrentUser', {}).then(function (data) {
                $scope.currentUser = data;
            }, function (errorMessage) {
              $confirm({
               unShowCancle: true,
               text: '用户未登录或无权执行该操作，请确认是否登录或更换账号后进行操作'
           }).then(function () {
               //$window.location.href = httpService.baseUri;
               location.href = "/login.html";
            ...
            $("#menu>li").click(function (event) {
                ...
                $("#menu li").eq($(this).index()).css("background", "none");
            ...
            $(function () {
                ...
                if (last == "adContentManage") {
                    ...
                    $("#menu>li").eq(5).children("span.lineIcon").css("display", "inline-block");
                } else if (last == "configManage") {
                    $("#menu>li").eq(5).children("span.lineIcon").css("display", "inline-block");
                } else if (last == "newsRelease") {
                    $("#menu>li").eq(5).children("span.lineIcon").css("display", "inline-block");
                }
