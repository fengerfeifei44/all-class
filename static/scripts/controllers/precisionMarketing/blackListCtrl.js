    ...
    function ($scope, httpService, Notification, $confirm, $uibModal) {
      ...
      //新增黑名单
        ...
        $scope.saveAddBlack = function () {
            ...
            if (reUsrId.test($scope.idAdd)) {
                //var aryM = $scope.idAdd.match(reUsrId);
                //if (!(aryM[1] < 256 && aryM[2] < 256 && aryM[3] < 256 && aryM[4] < 256)) {
                //    Notification.error({ message: "用户ID必须为正确的IP地址，如：10.3.45.10", delay: 3000 });
                //    return false;
                //}
            ...
            //    return false;
            //}
            if ($scope.comReasonAdd != "" && $scope.comReasonAdd != null) {
                if (!regText.test($scope.comReasonAdd)) {
                    Notification.error({ message: "投诉原因由字母、数字、下划线、中文组成,", delay: 3000 });
                    return false;
                }
            } 
            if ($scope.commentInfoAdd != "" && $scope.commentInfoAdd != null) {
