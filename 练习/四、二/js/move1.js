...
(function(){
    ...
    function move1(opt){
        if(!opt.id)return;//如果用户没有传id，直接返回；
        var tmpEffect=null;
        var defaultOpt={
            duration:1000,
            effect:0
        ...
        for(var attr in opt){
            defaultOpt[attr]=opt[attr]//把传进来的赋值给defaultOpt和下面的代码跟opt没有任何关系了
        }
        var ele=defaultOpt.id;
        ...
        if(typeof effect=='number'){
            ...
            tmpEffect=ary.length>=2?zhufengEffect[effect[0]][effect[1]]:zhufengEffect[effect[0]];
        }
