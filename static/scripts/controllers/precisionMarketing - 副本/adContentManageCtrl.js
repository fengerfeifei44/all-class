define(['scripts/services/httpService', 'scripts/requireHelper/requireNotification', 'bootstrap'], function () {
    return ['$scope', 'httpService', 'Notification', function ($scope, httpService, Notification) {
        ...
        $scope.submit = function () {
           ...
           window.intervalDeal = parseInt($scope.interval);
            ...
            if (reg.test($scope.interval)) {
                httpService.post("DAAS", 'ADAccountService', 'UpdateInterval', { dict_id: $scope.intervalDeal }).then(function (data) {
        ...
        $scope.verifyClick = function ($event,status,imgurlstr,Hrefstr,CHARGING_TYPE,show_num) {
            if (status != 4) return;
            $('#myModal').modal('show');
        ...
        $scope.agreeClick = function () {
             httpService.get("DAAS", 'ADAccountService', 'UpdateAdStatus', { ad_id: $scope.ad_id, ad_status:'0',type:$scope.ad_charging_type,show_num:$scope.ad_show_num}).then(function (data) {
        ...
        $scope.disagreeClick = function () {
            ...
            $('#myModal').modal('hide');
            httpService.get("DAAS", 'ADAccountService', 'UpdateAdStatus', { ad_id: $scope.ad_id, ad_status: '5', AD_COMMENTS: $scope.AD_COMMENTS, CHARGING_TYPE: $scope.ad_charging_type, show_num: $scope.ad_show_num }).then(function (data) {
